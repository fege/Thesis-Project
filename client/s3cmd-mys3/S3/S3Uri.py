...
class S3UriS3(S3Uri):
	...
	def host_name(self):
		if self.is_dns_compatible():
			return "%s.%s" % (self._bucket,Config.Config().host_base)
		else:
			return Config.Config().host_base
	...
	def httpurl_to_s3uri(http_url):
		...
		if hostname == Config.Config().host_base:
		...
		elif hostname.endswith("."+Config.Config().host_base):
			## new-style url: http://bucket.s3.amazonaws.com/object
			bucket = hostname[:-(len("."+Config.Config().host_base))]
		else:
